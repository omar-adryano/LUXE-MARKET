...
async function run() {
    ...
    const products = await mongoose.connection.collection('products').find({ source: 'cj' }).toArray();
    const caches = await mongoose.connection.collection('shippingcaches').find({}).toArray();
