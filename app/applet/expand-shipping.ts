...
async function run() {
  ...
  console.log(`Processing ${productsToProcess.length} products for ${targetCountries.length} countries...`);
  ...
  for (const product of productsToProcess) {
    for (const country of targetCountries) {
      ...
      if (exists) {
        // console.log(`[SKIP] Cache exists for VID: ${product.vid} | Country: ${country}`);
        continue;
      }
  ...
  const coveragePercent = ((totalCaches / (products.length * (targetCountries.length + 1))) * 100).toFixed(2);
