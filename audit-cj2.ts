...
// We have to copy CJDropshippingService.getProductInfo in case it's a module issue
...
async function run() {
  ...
  let count = state.processedIds.length;
  const startTime = Date.now();
  const TIME_LIMIT = 45000; // 45 seconds
  ...
  console.log('Processed', state.processedIds.length, '/', totalActive);
  process.exit();
}
