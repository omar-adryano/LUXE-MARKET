...
const authLogic = `import React, { useState } from 'react';
    ...
    <div className="flex min-h-screen w-full bg-slate-50 dark:bg-zinc-950 font-sans">
      ...
      <div className="flex w-full lg:w-2/5 flex-col justify-center px-4 py-12 sm:px-6 lg:px-20 xl:px-24">
        <div className="mx-auto w-full max-w-sm lg:max-w-md">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 shadow-xl shadow-slate-200/40 lg:border-none lg:bg-transparent lg:p-0 lg:shadow-none dark:border-zinc-800 dark:bg-zinc-900/80 lg:dark:bg-transparent lg:dark:shadow-none dark:shadow-none backdrop-blur-sm lg:backdrop-blur-none">
              ...
              <form onSubmit={handleLogin} className="space-y-5">
                ...
                <div className="relative mt-8 mb-6">
                  ...
                  <div className="relative flex justify-center text-sm font-medium leading-6">
                    <span className="bg-white px-4 text-slate-500 dark:bg-zinc-900/80 dark:text-zinc-400 lg:bg-slate-50 lg:dark:bg-zinc-950 rounded-full">Or continue with</span>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <button type="button" className="flex w-full items-center justify-center space-x-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700 transition-all">
                  ...
                  <button type="button" className="flex w-full items-center justify-center space-x-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700 transition-all">
                    <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none">
                      <path d="M7.076 21.337H2.47a.641.641 0 0 1-.633-.74l3.144-20.106A.641.641 0 0 1 5.613 0h7.136c3.23 0 5.613.534 6.81 1.83 1.112 1.11 1.583 2.766 1.341 4.717-.468 3.65-2.43 6.435-5.655 8.01-1.393.682-3.078 1.026-5.02 1.026v.004H8.441a.846.846 0 0 0-.834.707l-1.53 9.77a.64.64 0 0 1-.632.545z" fill="#003087"/>
                      <path d="M6.443 21.337h4.606a.64.64 0 0 0 .633-.74l2.126-13.57a.846.846 0 0 1 .834-.707h1.785c1.942 0 3.627-.344 5.02-1.026C19.74 3.754 18.91 2.3 17.51 1.365a12.63 12.63 0 0 0-4.757-.899H5.616a.64.64 0 0 0-.632.535L1.84 21.107a.64.64 0 0 0 .633.74h3.97z" fill="#009CDE"/>
                      <path d="M19.467 9.873c-.453 3.51-2.316 6.1-5.325 7.42-.999.444-2.138.647-3.398.647h-1.58q-.43.01-.522.383l-1.02 6.544a.64.64 0 0 1-.633.541H2.47a.64.64 0 0 1-.632-.741l1.454-9.332a.64.64 0 0 1 .632-.54h5.289c1.921 0 3.498-.328 4.716-1.021 2.302-1.325 3.77-3.486 4.225-6.06z" fill="#012169"/>
                    </svg>
                    <span>PayPal</span>
                  </button>
                </div>
            ...
            {authView === 'verify' && (
              <form onSubmit={handleVerify} className="space-y-5 font-sans">
                ...
                {(import.meta as any).env?.DEV && capturedVerifyPin && (
                  <div className="mb-6 rounded-2xl border border-amber-200 bg-amber-50 p-5 shadow-sm dark:border-amber-900/40 dark:bg-amber-900/10">
                    <div className="flex items-center space-x-2 text-amber-700 dark:text-amber-500 mb-2">
                      <Sparkles className="h-5 w-5 flex-shrink-0" />
                      <span className="font-semibold text-sm">Developer Assistant</span>
                    </div>
                    <button
                      type="button"
                      onClick={() => setVerificationCode(capturedVerifyPin)}
                      className="inline-flex items-center rounded-lg bg-amber-100 px-3 py-1.5 text-xs font-semibold text-amber-800 hover:bg-amber-200 dark:bg-amber-900/60 dark:text-amber-300 transition-colors"
                    >
                ...
                {(import.meta as any).env?.DEV && capturedResetPin && (
                  <div className="mb-6 rounded-2xl border border-amber-200 bg-amber-50 p-5 shadow-sm dark:border-amber-900/40 dark:bg-amber-900/10">
                    ...
                    <button
                      ...
                      className="inline-flex items-center rounded-lg bg-amber-100 px-3 py-1.5 text-xs font-semibold text-amber-800 hover:bg-amber-200 dark:bg-amber-900/60 dark:text-amber-300 transition-colors"
                    >
                      <span>Auto-fill PIN: {capturedResetPin}</span>
                    </button>
                  </div>
                )}
...
const fs = require('fs');
fs.writeFileSync('src/components/pages/Auth.tsx', authLogic + authUI);
console.log("Auth.tsx generated successfully");
